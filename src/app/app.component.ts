...
import { AddToClient } from './components/add-to-client';
...
export class AppComponent {
            ...
            this.articleService
              ...
              .subscribe(
                (result) => {
                  ...
                  this.getAllSectors();
                  // Process the result
                  // this.journalistNameArray = result.flatMap((item: any) =>
                  //   (item.journalists || []).map((j: any) => ({
                  ...
                  // .flatMap((item: any) => item.journalists || [])
                  // .map((j: any) => ({
                  //   Fname: (j.journalist || '').trim()
                  // }))
                  // .filter((j: any) => j.Fname !== '')
                  // .filter((value: any, index: number, self: any[]) =>
                  //   index === self.findIndex((t: any) => t.Fname === value.Fname)
                  // );
                  this.journalistNameArray = result
                    .map((item: any) => ({
                      JournalistID: item.JournalistID,
  ...
  // getJournalistNames(): { name: string }[] {
  //   return this.journalistNameArray.map(j => ({
  //     name: j.Fname   // using Fname as full name
  //   }));
  // }
  getJournalistNames(): {
    Fname: string;
  ...
  onSectorIdChange(event: Event): void {
    ...
    // const selectedSector = this.sectorsArray.find(
    //   (s: any) => s.ID === selectedSectorId
    // );
    // console.log('Selected sectorID:', selectedSectorId);
    this.article.SectorPid = selectedSectorId;
    // this.article.SectorName = selectedSector ? selectedSector.Name : '';
    // this.articleService
  ...
  // updateArticleMongo(): Promise<any> {
  //   return new Promise((resolve, reject) => {
  //     this.articleService
  //       .editArticleMongo(
  //         this.selectedArticleId,
  ...
  //         this.article.IsColor,
  //         this.UserID,
  //         this.article.SectorName,
  //         this.article.Page_Number,
  //         this.newPageNumber,
  //         this.newPageName,
  //         this.article.full_text,
  //         this.journalistNameArray.map(j =>
  //           `${j.Fname}${j.Lname ? ' ' + j.Lname : ''}`
  //         ) 
  //       )
  //       .subscribe(
  //         (result) => {
  ...
  sendMail() {
    ...
    body += `${this.fixEncoding(this.article.ArticleTitle)}\n\n`;
    body += `Add keywords: \n\n`;
    body += `Remove keyword:`;
  ...
  private buildClientPayload(selectedKeywords: any[]) {
    ...
    selectedKeywords.forEach(kw => {
      if (!map[kw.clientid]) {
        map[kw.clientid] = {
          ...
          clientname: kw.Clients,
          keyword: []
        };
      }

      map[kw.clientid].keyword.push({
        keyword: kw.Keyword,
        keyid: String(kw.KeywordID),
        keytpe: kw.keytype,
        companys: kw.companys ?? "null",
        brandString: kw.brands ?? "",
        keywordcategory: kw.keycategory,
        positivescore: "",
        negativescore: "",
        Sentiment: "",
        Prominence: "null",
  ...
  addToClient() {
    const modalRef = this.modalService.open(AddToClient, { size: 'xl' });
    modalRef.componentInstance.submitEvent.subscribe((res: any) => {
      const { selectedKeywords } = res;
      if (!selectedKeywords?.length) return;

      const clientPayload = this.buildClientPayload(selectedKeywords);
      ...
      console.log('Payload for addToClient:', payload);
      this.articleService.addToClient(payload).subscribe({
          next: async (res: any) => {
            console.log('Article added to client successfully:', res);
